...
#Control code
	MOVE_CODE: .asciiz "1b4"  # ma dieu khien duoc quy dinh truoc
...
#-------------------------------------------------------------------------------
	inputControlCode: .space 50     #luu lenh dang nhap tren digital lab sim
	lengthControlCode: .word 0      #luu do dai dang nhap trên digital lab sim
	nowHeading: .word 0		# cai dau ve vet cua bot
#---------------------------------------------------------
...
#---------------------------------------------------------
	path: .space 600       #lưu các lệnh đã nhập với mục đích go back
...
CheckControlCode:
		...
		lw $s2, 0($s2)
		#----------------
		bne $s2, 3, pushErrorMess  #neu ma khong bang 3 cai loi luon
		
		la $s3, MOVE_CODE
		jal isEqualString       #kiểm tra các ký tự mình nhập vào có đúng quy định không?
		beq $t0, 1, go
...
continue:     # de xoa cai ma dia chi sai 
...
go_to_first_point_of_edge:	
	...
	nop
	bne $t8, $t9, go_to_first_point_of_edge
	#khi nao bang moi xet tiep den y
	...
	beq $s5, 0, finish     #neu chay het cac canh ma structure ve 0 thi finish vi da ve diem xuat phat
	nop
	beq $s5, 0, finish
...
isEqualString:
	...
	addi $t1, $zero, -1				#$t1 = -1 = i
	add $t0, $zero, $zero
	la $s1, inputControlCode			#$s1 khai bao dia chi cua thanh ghi s1 la phan tu dau tien cua mang
	for_loop_to_check_equal:
		...
		add $t3, $s3, $t1			#$t3 = s + i   S3 la dia chi cua ma dieu khien
		lb $t3, 0($t3)				#$t3 = s[i]
...
# NGĂT:( NHẬN GIÁ TRỊ VÀO SAO LƯU VÀO INPUTCONTROLCODE), CẬP NHẬT ĐỘ DÀI LENGTHCONTROLCODE
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
.ktext 0x80000180
#-------------------------------------------------------
